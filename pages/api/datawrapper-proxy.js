...
export default async function handler(req, res) {
  const { csv, indicator, location, chartType, minValue, maxValue, rankMode } = JSON.parse(req.body);

  const chartIdAndUrl = await dataVisualiser(csv, indicator, location, chartType, minValue, maxValue, rankMode);
